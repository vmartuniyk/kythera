    ...
    static function ParseDate($date)
    {
        ...
        switch ($cal) {
            case '@#NUMERICGREGORIANDATE@':
                return new NumericGregorianDate([$y, $m, $d]);
            case '@#DGREGORIAN@':
                return new WT_Date_Gregorian([$y, $m, $d]);
            case '@#DJULIAN@':
                return new WT_Date_Julian([$y, $m, $d]);
            case '@#DHEBREW@':
                return new WT_Date_Jewish([$y, $m, $d]);
            case '@#DHIJRI@':
                return new WT_Date_Hijri([$y, $m, $d]);
            case '@#DFRENCH R@':
                return new WT_Date_French([$y, $m, $d]);
            case '@#DJALALI@':
                return new WT_Date_Jalali([$y, $m, $d]);
            case '@#DROMAN@':
                return new WT_Date_Roman([$y, $m, $d]);
        }
    }
    ...
    function Display($url = false, $date_fmt = null, $cal_fmts = null)
    {
        ...
        switch ($q1.$q2) {
            case '':
                $tmp=$d1.$conv1;
                break;
            case 'ABT':    /* I18N: Gedcom ABT dates     */
                $tmp=WT_I18N::translate('about %s', $d1.$conv1);
                break;
            case 'CAL':    /* I18N: Gedcom CAL dates     */
                $tmp=WT_I18N::translate('calculated %s', $d1.$conv1);
                break;
            case 'EST':    /* I18N: Gedcom EST dates     */
                $tmp=WT_I18N::translate('estimated %s', $d1.$conv1);
                break;
            case 'INT':    /* I18N: Gedcom INT dates     */
                $tmp=WT_I18N::translate('interpreted %s (%s)', $d1.$conv1, $this->text);
                break;
            case 'BEF':    /* I18N: Gedcom BEF dates     */
                $tmp=WT_I18N::translate('before %s', $d1.$conv1);
                break;
            case 'AFT':    /* I18N: Gedcom AFT dates     */
                $tmp=WT_I18N::translate('after %s', $d1.$conv1);
                break;
            case 'FROM':   /* I18N: Gedcom FROM dates    */
                $tmp=WT_I18N::translate('from %s', $d1.$conv1);
                break;
            case 'TO':     /* I18N: Gedcom TO dates      */
                $tmp=WT_I18N::translate('to %s', $d1.$conv1);
                break;
            case 'BETAND': /* I18N: Gedcom BET-AND dates */
                $tmp=WT_I18N::translate('between %s and %s', $d1.$conv1, $d2.$conv2);
                break;
            case 'FROMTO': /* I18N: Gedcom FROM-TO dates */
                $tmp=WT_I18N::translate('from %s to %s', $d1.$conv1, $d2.$conv2);
                break;
            default:
