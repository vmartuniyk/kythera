/**
 * @preserve Basic Primitives orgDiagram v2.0.24
                ...
                if (annotationConfig.items != null && annotationConfig.items.length > 0) {
                    ...
                    if (items.length > 1) {
                        for (index2 = 1, len2 = items.length; index2 < len2; index2 += 1) {
                            ...
                            path = primitives.common.getShortestPath(graph, firstItem, nextItem, function (edge) {
                                return edge.weight;
                            });//ignore jslint
    ...
    for (index = 0, len = this._treeLevels.length; index < len; index += 1) {
        ...
        for (index2 = 0, len2 = treeLevel.treeItems.length; index2 < len2; index2+=1) {
            ...
            for (index3 = 0, len3 = toItems.length; index3 < len3; index3 += 1) {
                ...
                treeItemTo = this._treeItems[itemTo];
                treeItemToWeight = treeItemTo.actualIsActive ? 100 : 1;

                if (!result.hasOwnProperty(treeItem.id)) {
                    result[treeItem.id] = {};
                }
                result[treeItem.id][itemTo] = (len3 == 1) ? { id: treeItem.id, weight: treeItemToWeight } : { id: treeItem.id, partnerid: itemTo, weight: treeItemToWeight };

                if (!result.hasOwnProperty(itemTo)) {
                    result[itemTo] = {};
                }
                result[itemTo][treeItem.id] = (len3 == 1) ? { id: treeItem.id, weight: treeItemWeight } : { id: treeItem.id, partnerid: itemTo, weight: treeItemWeight };
            }
        }
    }
    return result;
            ...
            case 4/*primitives.common.AnnotationType.Background*/:
                if (annotationConfig.items != null && annotationConfig.items.length > 0) {
                    ...
                    if (annotationConfig.includeChildren) {
                        ...
                        treeItems = this._getAllLogicalChildren(treeItems, 'logicalChildren');
                        for (index2 = 0, len2 = treeItems.length; index2 < len2; index2 += 1) {
                            treeItem = treeItems[index2];
    ...
    if (this._treeLevels.length > 0) {
        ...
        for (index = 0, len = this._treeLevels.length; index < len; index += 1) {
            ...
            if (treeLevel.actualVisibility != 4/*primitives.common.Visibility.Invisible*/) {
                ...
                for (index2 = 0, len2 = treeItems.length; index2 < len2; index2 += 1) {
                    nextTreeItem = this._treeItems[treeItems[index2]];
...
primitives.orgdiagram.BaseController.prototype._getAllLogicalChildren = function (treeItems) {
    return this._getAllLogicalParents(treeItems, 'logicalChildren');
};

primitives.orgdiagram.BaseController.prototype._getAllLogicalParents = function (treeItems, collectionName) {
    var result = [],
        parents = [],
        parent,
        newParents = [],
    ...
    collectionName = collectionName || 'logicalParents';

    for (index2 = 0, len2 = treeItems.length; index2 < len2; index2 += 1) {
        treeItem = treeItems[index2];

        if (!hash.hasOwnProperty(treeItem.id)) {
            hash[treeItem.id] = true;
            result.push(treeItem);

            parents = parents.concat(treeItem[collectionName]);
            while (parents.length > 0) {
                for (index = 0, len = parents.length; index < len; index += 1) {
                    parent = this._treeItems[parents[index]];
                    result.push(parent);
                    newParents = newParents.concat(parent[collectionName]);
                }
                parents = newParents;
                newParents = [];
            }
...
*/
primitives.common.getShortestPath = function (graph, startNode, endNode, getWeightFunc) {
    var margin = {},
        distance = {},
        breadcramps = {},
        marginLength = 0,
    ...
    while (marginLength > 0) {
        ...
        for (key in children) {
            if (children.hasOwnProperty(key)) {
                newDistance = bestDistanceToNode + (getWeightFunc!=null ? getWeightFunc(children[key]) : 1);
...
primitives.famdiagram.Controller.prototype._virtualCreateOrgTree = function () {
    var index, len,
        ...
        self = this,
        parsedId,
        childrenHash, parentsHash,
        labelConfigsHash,
        labelConfigs, labelConfig,
        itemsHavingSpouses, itemHavingSpouse, spouses, spouses2, spouseid, hasCommonChild, childItemIsCommon, fakeChild;
    ...
    if (this.options.items.length > 0) {
        ...
        for (itemId in this._famItems) {
            if (this._famItems.hasOwnProperty(itemId)) {
                ...
                if (parents != null) {
                    for (index = 0, len = parents.length; index < len; index += 1) {
                        parentItemId = parents[index];
                        if (this._famItems.hasOwnProperty(parentItemId) && parentItemId != famItem.id && !duplicates.hasOwnProperty(parentItemId)) {

                            duplicates[parentItemId] = true;

                            parentItem = this._famItems[parentItemId];

                            parentItem.children.push(famItem.id);
                            parentItem.logicalChildren.push(famItem.id);
                            famItem.logicalParents.push(parentItemId);
                            famItem.parents.push(parentItemId);

                            if (!childrenHash.hasOwnProperty(parentItemId)) {
                                childrenHash[parentItemId] = {};
                            }
                            childrenHash[parentItemId][itemId] = parentItemId;

                            if (!parentsHash.hasOwnProperty(itemId)) {
                                parentsHash[itemId] = {};
                            }
                            parentsHash[itemId][parentItemId] = itemId;
                        }
                    }
                }
        ...
        for (index = 0, len = itemsHavingSpouses.length; index < len; index += 1) {
            itemHavingSpouse = itemsHavingSpouses[index];
            duplicates = {};
            duplicates[itemHavingSpouse.id] = true;

            spouses2 = [itemHavingSpouse.id];
            spouses = itemHavingSpouse.itemConfig.spouses;
            for (index2 = 0, len2 = spouses.length; index2 < len2; index2 += 1) {
                spouseid = spouses[index2];

                if (this._famItems.hasOwnProperty(spouseid) && !duplicates.hasOwnProperty(spouseid)) {
                    duplicates[spouseid] = true;
                    spouses2.push(spouseid);
                }
            }

            hasCommonChild = false;
            for (index3 = 0, len3 = itemHavingSpouse.logicalChildren.length; index3 < len3; index3 += 1) {
                childItem = this._famItems[itemHavingSpouse.logicalChildren[index3]];

                if (childItem.logicalParents.length == spouses2.length) {
                    childItemIsCommon = true;
                    for (index4 = 0, len4 = childItem.logicalParents.length; index4 < len4; index4 += 1) {
                        if (!duplicates.hasOwnProperty(childItem.logicalParents[index4])) {
                            childItemIsCommon = false;
                            break;
                        }
                    }
                    if (childItemIsCommon) {
                        hasCommonChild = true;
                        break;
                    }
                }
            ...
            if (!hasCommonChild) {
                ...
                this._famItems[fakeChild.id] = fakeChild;

                for (index2 = 0, len2 = spouses2.length; index2 < len2; index2 += 1) {
                    spouseid = spouses2[index2];
                    parentItem = this._famItems[spouseid];

                    parentItem.logicalChildren.push(fakeChild.id);
                    parentItem.children.push(fakeChild.id);
                    fakeChild.logicalParents.push(parentItem.id);
                    fakeChild.parents.push(parentItem.id);

                    if (!childrenHash.hasOwnProperty(parentItem.id)) {
                        childrenHash[parentItem.id] = {};
                    }
                    childrenHash[parentItem.id][fakeChild.id] = parentItem.id;

                    if (!parentsHash.hasOwnProperty(fakeChild.id)) {
                        parentsHash[fakeChild.id] = {};
                    }
                    parentsHash[fakeChild.id][parentItem.id] = fakeChild.id;
                }
            }
        }
        ...
        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                ...
                famItem.logicalChildren.sort(function (a, b) { return self._famItems[a].level - self._famItems[b].level; });//ignore jslint
                famItem.logicalParents.sort(function (a, b) { return self._famItems[b].level - self._famItems[a].level; });//ignore jslint
            }
        }

        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                this._famItems[key].level = null;
            }
        }

        this._sortItemsBylevelsSmart(this._famItems);

        /* Add labels annotations to hierarchy. */
        if (this.options.annotations.length > 0) {
            for (key in this._famItems) {
                if (this._famItems.hasOwnProperty(key)) {
                    famItem = this._famItems[key];

                    famItem.logicalChildren.length = 0;
                    famItem.logicalParents.length = 0;
                    famItem.children.length = 0;
                    famItem.parents.length = 0;
                }
            }

            labelConfigsHash = {};
            for (index = 0, len = this.options.annotations.length; index < len; index += 1) {
                labelConfig = this.options.annotations[index];
                if (labelConfig.annotationType == 3/*primitives.common.AnnotationType.Label*/) {
                    if (!labelConfigsHash.hasOwnProperty(labelConfig.fromItem)) {
                        labelConfigsHash[labelConfig.fromItem] = [labelConfig];
                    } else {
                        labelConfigsHash[labelConfig.fromItem].push(labelConfig);
                    }
                }
            }

            for (fromItem in labelConfigsHash) {
                if (labelConfigsHash.hasOwnProperty(fromItem)) {
                    labelConfigs = labelConfigsHash[fromItem];
                    labelConfigs.sort(function (a, b) {
                        return b.toItems.length - a.toItems.length;
                    }); //ignore jslint


                    for (index = 0; index < labelConfigs.length; index += 1) {
                        labelConfig = labelConfigs[index];

                        this._addLabelAnnotation(childrenHash, labelConfig.fromItem, labelConfig.toItems, function () {
                            var result = null;
                        ...
                        this._addLabelAnnotation(parentsHash, labelConfig.fromItem, labelConfig.toItems, function () {
                            var result = null;
                            /* add label annotation as new diagram family item */
                            self.maximumFamItemId += 1;
            ...
            /* convert hashs to parents, children, logicalChildren */
            for (itemId in this._famItems) {
                if (this._famItems.hasOwnProperty(itemId)) {
                    famItem = this._famItems[itemId];

                    children = childrenHash[famItem.id];
                    if (children != null) {
                        for (childItemId in children) {
                            if (children.hasOwnProperty(childItemId)) {
                                parentItem = famItem;
                                if (children[childItemId] != famItem.id) {
                                    parentItem = this._famItems[children[childItemId]];
                                }

                                childItem = this._famItems[childItemId];
                                if (parentsHash.hasOwnProperty(childItemId)) {
                                    if (parentsHash[childItemId].hasOwnProperty(famItem.id)) {
                                        if (parentsHash[childItemId][famItem.id] != childItemId) {
                                            childItem = this._famItems[parentsHash[childItemId][famItem.id]];
                                        }
                                    }
                                }

                                parentItem.logicalChildren.push(childItem.id);
                                childItem.logicalParents.push(parentItem.id);
                                parentItem.children.push(childItem.id);
                                childItem.parents.push(parentItem.id);
                            }
                        }
                    }

                    parents = parentsHash[famItem.id];
                    if (parents != null) {
                        for (parentItemId in parents) {
                            if (parents.hasOwnProperty(parentItemId)) {
                                parentItem = this._famItems[parentItemId];
                                childItem = famItem;

                                if (parents[parentItemId] == famItem.id) {
                                    if (!childrenHash.hasOwnProperty(parentItemId)) {
                                        parentItem.logicalChildren.push(childItem.id);
                                        childItem.logicalParents.push(parentItem.id);
                                        parentItem.children.push(childItem.id);
        ...
        this._eliminateManyToMany(this._famItems,
            function () {
                ...
                result = new primitives.famdiagram.FamilyItem({
                    ...
                    isVisible: false,
                    isActive: false,
                    itemConfig: new primitives.famdiagram.ItemConfig({ id: self.maximumFamItemId }),
                    levelGravity: 2/*primitives.common.GroupByType.Children*/
                });
        ...
        this._orgItemRoot.isActive = false;
...
primitives.famdiagram.Controller.prototype._addLabelAnnotation = function (hash, fromItem, toItems, onCreate) {
    var children,
        isValid,
        commonParentId,
        toItem,
        index2,
        familyItemId;
    if (hash.hasOwnProperty(fromItem)) {
        children = hash[fromItem];

        isValid = true;
        commonParentId = null;
        for (index2 = 0; index2 < toItems.length; index2 += 1) {
            toItem = toItems[index2];

            if (children.hasOwnProperty(toItem)) {
                if (commonParentId == null) {
                    commonParentId = children[toItem];
                } else {
                    if (commonParentId != children[toItem]) {
                        isValid = false;
                        break;
                    }
                }
        ...
        if (isValid) {
            familyItemId = onCreate();

            /* update hashes */
            if (!hash.hasOwnProperty(commonParentId)) {
                hash[commonParentId] = {};
            }
            hash[commonParentId][familyItemId] = commonParentId;

            for (index2 = 0; index2 < toItems.length; index2 += 1) {
                toItem = toItems[index2];

                children[toItem] = familyItemId;
            }
        }
    }
};
...
primitives.famdiagram.Controller.prototype._sortItemsBylevelsSmart = function (famItems) {
    var usedEdgesHash = {},
        itemid, famItem;
    /* Distribute this._famItems by levels. */
    for (itemid in famItems) {
...
primitives.famdiagram.Controller.prototype._sortItemsBylevelsTopo = function (famItems) {
    var nodes = {},
        key, sortedNodes,
